...
import com.ioomex.olecodeApp.mapper.PostMapper;
...
public class PostServiceImpl extends ServiceImpl<PostMapper, Post> implements PostService {
    ...
    public PostVO getPostVO(Post post, HttpServletRequest request) {
        ...
        // 2. 已登录，获取用户点赞、收藏状态
        SysUser loginSysUser = userService.getLoginUserPermitNull(request);
        if (loginSysUser != null) {
            // 获取点赞
            QueryWrapper<PostThumb> postThumbQueryWrapper = new QueryWrapper<>();
            postThumbQueryWrapper.in("postId", postId);
            postThumbQueryWrapper.eq("userId", loginSysUser.getId());
            PostThumb postThumb = postThumbMapper.selectOne(postThumbQueryWrapper);
            postVO.setHasThumb(postThumb != null);
            // 获取收藏
            QueryWrapper<PostFavour> postFavourQueryWrapper = new QueryWrapper<>();
            postFavourQueryWrapper.in("postId", postId);
            postFavourQueryWrapper.eq("userId", loginSysUser.getId());
    ...
    public Page<PostVO> getPostVOPage(Page<Post> postPage, HttpServletRequest request) {
        ...
        List<PostVO> postVOList = postList.stream().map(post -> {
            ...
            postVO.setUser(userService.getUserVO(sysUser));
